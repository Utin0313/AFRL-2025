...
t_pause = 3       # Delay time (sec) between iterations
...
# Configuration 1
print("Setting Amplitude (0.9, 0.5, 0.1): ")
xmlrpc_control.set_amp0(0.9) # Set filter config
xmlrpc_control.set_amp1(0.5) # Set filter config
xmlrpc_control.set_amp2(0.1) # Set filter config
time.sleep(t_pause)
P_avg = zmq_measure(socket1,PRINT_MEASUREMENTS)
print(f"  Current Prediction: {P_avg:.4}")
time.sleep(t_pause)


# Configuration 1
print("Setting Amplitude (0.1, 0.5, 0.1): ")
xmlrpc_control.set_amp0(0.1) # Set filter config
xmlrpc_control.set_amp1(0.5) # Set filter config
xmlrpc_control.set_amp2(0.1) # Set filter config
time.sleep(t_pause)
P_avg = zmq_measure(socket1,PRINT_MEASUREMENTS)
...
# Configuration 1
print("Setting Amplitude (0.1, 0.2, 0.9): ")
xmlrpc_control.set_amp0(0.1) # Set filter config
xmlrpc_control.set_amp1(0.2) # Set filter config
xmlrpc_control.set_amp2(0.9) # Set filter config
time.sleep(t_pause)
P_avg = zmq_measure(socket1,PRINT_MEASUREMENTS)
print(f"  Current Prediction: {P_avg:.4}")
time.sleep(t_pause)
